...
def get_last_name_count(list_of_records):
    ...
    """
    ## Write code below ##
    list_of_last_names = [] #create an empty list
    for item in list_of_records: # for each item in list of records
        email, full_name = item #save the email and full name into their own variables
        list_of_last_names.append(full_name['last_name']) # use the append method on the new list to add the last name key/value pair from passed in list
    return Counter(list_of_last_names) # the counter function counts how many occurences happen in the list
    ## end of function ##
...
def update_record(list_of_records, email, first_name, last_name):
    ...
    ## Write code below ##
    found_email = False
    for index, record in enumerate(list_of_records): # for each list value we want the record info and the index its located at
        record_email, full_name = record # we break up the record value into email and name
        if record_email == email: # compare the record email to the user inputted email
            found_email = True # if it exists, we want to set our variable to true
            break # then we want to escape our for loop
    if found_email: #once for loop is exited we are checking for true or false
        list_of_records[index] = (email, {'first_name':first_name, 'last_name':last_name}) #if true update the record's first name and last name value  pairs
    else:
        list_of_records.append((email, {'first_name':first_name, 'last_name':last_name})) # if false add new record
